...
namespace
{
      ...
      constexpr Matrix()
      {
        ...
        for (std::size_t row = 0; row < Rows; ++row)
        {
          ...
          for (auto const c : Data[row])
          {
            ...
            for (int i = 3; i >= 0; --i)
            {
              if (bitIndex < Cols) set(row, bitIndex++, (value >> i) & 1);
            }
          }
        }
      }
  ...
  // it into tones. Index of the Costas array set to use is specified
  // by the caller, as is the type of message, indicated by the lower
  // 3 bits of `type`.
