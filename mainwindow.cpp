...
namespace
{
         ...
         int  const type)
  {
    ...
    // Encode the message into a 174-bit codeword. Here we're using a
    // parity generation matrix with dimensions (87, 87), generated by
    // the PEG algorithm, reordered to be a (174, 87) regular LDPC code
    // with column weight 3. Results here should be identical to what
    // the Fortran 'encode174' code produced, but by precomputing the
    // column order work at compile time, we avoid the temporary that
    // it uses.
